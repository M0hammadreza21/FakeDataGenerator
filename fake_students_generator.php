<?php
    ...
    $male_Firstname = array_rand($firstNameMale,1);
    $female_Firstname = array_rand($firstNameFemale,1);
    $generated_lastName = array_rand($lastName,1);
    
    $student_fullname = $firstNameFemale[$female_Firstname] .' '.$lastName[$generated_lastName];
    $student_dateofbirth = rand(1387,1389) .'/'. rand(1,12) .'/'. rand(1,30);
    $student_nationalCode = '197' .rand(200,500).''.rand(600,700).''.rand(1,9);
    $student_password = rand(102030,908070);
...
require_once "config.php";
        ...
        $stmt = $pdo->prepare("INSERT INTO students (student_fullname, student_dateofbirth, student_nationalCode, student_password) VALUES (?,?,?,?)"); 
        if($stmt->execute([ $student_fullname, $student_dateofbirth, $student_nationalCode, $student_password ])) {
                $stmt = null;
